...
bool UpdateExecutor::Next([[maybe_unused]] Tuple *tuple, RID *rid) {
    ...
    while(child_executor_->Next(&Old_tuple, rid)) {
        ...
        for(auto index_info : indexes) {
            Tuple key_tuple(Old_tuple.KeyFromTuple(table_info_->schema_, index_info->key_schema_, index_info->index_->GetKeyAttrs()));
            index_info->index_->DeleteEntry(key_tuple, *rid, txn);
            Tuple new_key_tuple(New_tuple.KeyFromTuple(table_info_->schema_, index_info->key_schema_, index_info->index_->GetKeyAttrs()));
